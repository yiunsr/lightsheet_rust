...
fn test_get_col_sep(){
    ...
    let sep1 = csv_manager::get_col_sep(&csv_str1);
    assert_eq!(b',', sep1);

    let csv_str2 = "a1\tb1\tc1\td1\na2\tb2\tc2\td2".to_string();
    let sep2 = csv_manager::get_col_sep(&csv_str2);
    assert_eq!(b'\t', sep2);

    let csv_str3 = "a1|b1|c1|d1\na2|b2|c2|d2".to_string();
    let sep3 = csv_manager::get_col_sep(&csv_str3);
    assert_eq!(b'|', sep3);
}
...
fn test_csv_open(){
    ...
    let result_open = csv_manager::csv_open(file);
...
fn test_read_csv(){
    ...
    csv_manager::read_csv(db_path, file_path, 1u32, test_callback);
}
...
fn test_read_csv_01(){
    ...
    let db_path = "D:\\workspace\\vscode\\lightsheet_rust\\tmp\\tt.db".to_string();
    let _ret = csv_manager::read_csv(db_path, file_path, 1u32, test_callback);
    // match ret {
    //     Ok(file) => file,
    //     Err(err) => {
    //         println!("Err:  {:?}", err);
