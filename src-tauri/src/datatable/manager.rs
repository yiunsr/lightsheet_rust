...
use super::db_utils;
use crate::datatable::csv_manager;
use crate::datatable::db_manager;
...
impl TableManager {
    ...
    pub fn new(db_path:String) -> TableManager
    {
        // 메모리 데이터베이스
    ...
    pub fn new_table(&mut self, window_id:u32){
        let conn = self.conn.as_mut().unwrap();
        let row_len = 5u32;
        let col_len = 5u32;
        db_manager::new_table(conn, window_id, row_len, col_len);
    ...
    pub fn get_file_info(&mut self, path: String) -> Value{
        csv_manager::get_file_info(path)
    }
        ...
        where F: Fn(u32) -> ()
    {
        ...
        let now = Instant::now();
        let conn = self.conn.as_mut().unwrap();
        let table_info = csv_manager::read_csv(conn, filepath, window_id, cb);
    ...
    pub fn get_rows(&mut self, window_id:u32, from:u32, to:u32) -> String{
        ...
        let conn = self.conn.as_mut().unwrap();
        db_manager::get_rows(&conn, window_id, 
            col_len, from, to)
    }
    ...
    pub fn add_rows(&mut self, window_id:u32, row_idx:u32, row_add_count:u32){
        ...
        let conn = self.conn.as_mut().unwrap();
        db_manager::add_rows(conn, window_id, row_idx, row_add_count, row_len, col_len);
    ...
    pub fn cell_edit(&mut self, window_id:u32, row_id:u32, col_index:u32,
        ...
        let conn = self.conn.as_mut().unwrap();
        db_manager::cell_edit(conn, window_id, row_id, col_index, &old_value, &new_value)
    }
